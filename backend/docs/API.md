...
**200** — 바뀐 정보를 그대로 돌려줍니다(다시 조회하지 않아도 됩니다).

| | |
|---|---|
| 안 보낸 항목 | **그대로 둡니다.** 이름만 바꾸려면 `{"name": "..."}` 만 보내세요 |
| `"phone": ""` | 연락처를 **지웁니다** |
| 이메일 | **바꿀 수 없습니다.** 로그인 아이디라 중복 확인·토큰 처리가 함께 필요합니다 |

## 1-5. 비밀번호 변경

```json
PUT /api/auth/me/password
{ "currentPassword": "지금비밀번호", "newPassword": "새비밀번호" }
```

**204** — 본문 없이 성공만 알려줍니다. **기존 토큰은 그대로 쓸 수 있어** 다시 로그인하지
않아도 됩니다.

| 상황 | 응답 |
|---|---|
| 현재 비밀번호가 틀림 | `400` + `"현재 비밀번호가 올바르지 않습니다."` |
| 새 비밀번호가 현재와 같음 | `400` + `"새 비밀번호가 현재 비밀번호와 같습니다."` |
| 새 비밀번호가 8자 미만 | `400` + `fields.newPassword` |

> **현재 비밀번호를 반드시 받습니다.** 토큰만으로 바꾸게 하면, 자리를 비운 사이 남이
> 브라우저를 만졌을 때 비밀번호까지 바뀌어 계정을 통째로 잃습니다.

> ⚠️ **비밀번호 찾기(재설정)는 아직 없습니다.** 이메일 발송이 필요해서 별도 논의 중입니다.
> 지금은 비밀번호를 잊으면 새 계정을 만들어야 합니다.
...
```json
{
  ...
  "severity": {
    ...
    "determined": true,                                 // 숫자로 판정했는지
    "matchedCriteria": ["3개월 이상 부상자 2명 이상"],     // 충족한 기준 (추정이면 [])
    "note": "입력하신 피해 규모가 중대재해 기준에 해당합니다 ...",
    "criteria": [
      "사망자가 1명 이상 발생한 재해",
...
**③ 피해 규모를 물어보시면 중대재해를 자동 판정합니다** ⭐

사망자·부상자 수를 입력받아 함께 보내면 **판정**하고(`determined: true`), 어느 기준에
걸렸는지(`matchedCriteria`)까지 알려드립니다. 기준은 DB 가 정본이라 법이 개정돼도 따라갑니다.

```
숫자 안 보냄  → determined: false · criteria 3가지를 체크리스트로 보여주고 직접 대조
숫자 보냄     → determined: true  · matchedCriteria 를 "이 기준에 해당합니다" 로 강조
```

숫자를 안 받으실 거면 기존처럼 `criteria` 를 체크리스트로 그리시면 됩니다.
글만으로는 "3개월 이상 요양" 같은 기준을 알 수 없어 **추정에 그칩니다.**
...
| **400** | 목록에 없는 코드값 | `{"error":"선택할 수 없는 값입니다. 목록에 있는 값인지 확인해 주세요.","fields":{"region":"..."}}` |
| **403** | 토큰 없음/만료 | (본문 없음) |
...
`industry` · `region` · `sizeClass` 는 **DB 코드 테이블에 있는 값만** 받습니다. 아래 목록과 정확히 같아야 합니다.

| 항목 | 허용값 |
|---|---|
| `industry` | 제조업 · 건설업 · 운수창고통신업 · 전기가스증기수도사업 |
| `region` | 서울 · 부산 · 대구 · 인천 · 광주 · 대전 · 울산 · 경기 · 강원 · 충북 · 충남 · 전북 · 전남 · 경북 · 경남 · 제주 |
| `sizeClass` | 5인 미만 · 5~9인 · 10~19인 · 20~29인 · 30~49인 · 50~99인 · 100~299인 · 300~499인 · 500~999인 · 1,000인 이상 |

`"부산광역시"` 처럼 조금이라도 다르면 400 이 나가고, `fields` 에 **어느 칸이 틀렸는지** 담아 보냅니다.
